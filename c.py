...
async def get_token(username, password):
    ...
    if response.status_code == 200:
        ...
        return {'token': token, 'user_id': user_id, 'username': username, 'api_data': api_data, 'ip': my_ip}
...
async def get_api_data(user_id, token):
    async with httpx.AsyncClient() as client:
        response = await client.get(
            f'https://swadbot.com/wp-json/sw/v1/apikey',
            params={'id': f'{user_id}'},
...
def get_session_id():
    ...
    if LIC_SESSION_ID == '':
        session_id = f'{machineid.id()}-{str(datetime.now().timestamp()).replace(".", "-")}'
